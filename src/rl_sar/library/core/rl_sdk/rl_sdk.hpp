...
namespace Input
{
    ...
    enum class Gamepad
    {
        ...
        LB_RB,
        LT_B,       // LT + B: poweroff
        RT_DPadUp,    // RT + DPadUp: ClimbWall
        RT_DPadDown,  // RT + DPadDown: Bridge
        Back_Start    // LB + A combination (Back + Start buttons)
    };
}
...
struct Control
{
    ...
    std::string nav_requested_state;  // 导航请求的目标状态（空字符串表示无请求）
    ...
    void ClearInput()
    {
        ...
        current_gamepad = last_gamepad;//current_gamepad = Input::Gamepad::None;
    }
};
...
public:
    ...
    bool is_simulation = false;         // 仿真模式标记（Sim 构造时置 true）
...
public:
    ...
    // 导航模式下检查是否有状态转换请求，排除 CalfSwing
    // 用法: if (TryNavStateChange("RLFSMStateGetUp")) return "RLFSMStateGetUp";
    bool TryNavStateChange(const std::string& target_state)
    {
        if (rl.control.navigation_mode && !rl.control.nav_requested_state.empty())
        {
            if (rl.control.nav_requested_state == target_state &&
                target_state != "RLFSMStateCalfSwing")
            {
                return true;
            }
        }
    ...
    // 检测所有电机是否掉线（位置连续不更新超过 threshold_frames 帧）
    // 在运动状态的 CheckSafety 中调用，防止电机意外掉线后 FSM 仍在发指令
    // 注意：仿真模式下始终返回 false（不检测），物理引擎不会真正掉线
    bool AreMotorsOffline(int stall_frames = 500, float threshold = 0.001f)
    {
        if (rl.is_simulation) return false;  // 仿真不检测

        const auto& q = fsm_state->motor_state.q;
        int n = static_cast<int>(q.size());
        if (n == 0) return true;

        if (stall_q_.size() != static_cast<size_t>(n))
        {
            stall_q_ = q;
            stall_counter_ = 0;
            return false;
        ...
        for (int i = 0; i < n; ++i)
        {
            if (std::abs(q[i] - stall_q_[i]) > threshold)
            {
                any_moved = true;
                break;
            }
        ...
        stall_q_ = q;
        stall_counter_++;
        if (stall_counter_ >= stall_frames)
        {
            std::cout << LOGGER::WARNING << "[Safety] All motors stalled for "
                      << stall_counter_ << "/" << stall_frames << " frames — suspected offline/disconnected." << std::endl;
            return true;
        }
        return false;
    }
...
protected:
    std::vector<float> stall_q_;    // 停转检测：上一帧电机位置
    int stall_counter_ = 0;         // 连续未移动帧数
};
